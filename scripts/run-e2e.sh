...
if ! cosign public-key --key ovhcloud://"$KMS_INTEGRATION_KEY_ID" > public.key; then
...
echo "[2/3] Signing..."
if ! cosign sign-blob --key ovhcloud://"$KMS_INTEGRATION_KEY_ID" --bundle test.bundle test.txt; then
...
if ! cosign verify-blob --key ovhcloud://"$KMS_INTEGRATION_KEY_ID" --bundle test.bundle test.txt; then
    echo "TEST FAILED: Verification with KMS key failed."
    exit 1
fi

echo "bad content" > bad.txt
if cosign verify-blob --key ovhcloud://"$KMS_INTEGRATION_KEY_ID" --bundle test.bundle bad.txt; then
